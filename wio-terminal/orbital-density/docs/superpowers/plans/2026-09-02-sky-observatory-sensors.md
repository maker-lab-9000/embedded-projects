...
bool magOk = false;
uint32_t lastPrint = 0, lastSetReset = 0, lastProbe = 0;
...
bool magInit() {
  ...
  mmc.magnetSetReset();         // degauss once (~2 ms)
  return true;
}
...
void loop() {
  ...
  if (millis() - lastSetReset >= 60000) { lastSetReset = millis(); mmc.magnetSetReset(); }
...
- `|B|` roughly 25–65 µT (Earth's field; indoors near steel it can be off, that is expected);
...
// library busy-waits in one-shot mode). SET/RESET every 60 s purges offset drift (~2 ms).
// Heading assumes the instrument is level; tilt compensation is future work (the Wio's
// built-in LIS3DHTR on Wire1 is the obvious source).
#pragma once
#include <Arduino.h>
#include <Adafruit_MMC56x3.h>
#include "ring.h"
...
const uint32_t MAG_SETRESET_MS = 60000;
...
Adafruit_MMC5603 mmc;
bool     magOk = false;
uint32_t magLastMs = 0, magLastSetResetMs = 0;
...
void magPoll() {
  ...
  magLastMs = now;
  if (now - magLastSetResetMs >= MAG_SETRESET_MS) {   // degauss; skip this sample slot
    magLastSetResetMs = now;
    mmc.magnetSetReset();
    return;
  }
  sensors_event_t e;
...
Upload, 5 minutes. Expected: `|B|` and `hdg` plausible and steady when still; `loopMax` and `nmeaFail` within baseline (the once-a-minute SET/RESET adds ~2 ms, invisible). Unplug/replug the MMC5603: values freeze then resume within 30 s.
